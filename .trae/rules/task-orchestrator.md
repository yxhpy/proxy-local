# TASK-ORCHESTRATOR Agent Rule

This rule is triggered when the user types `@task-orchestrator` and activates the Task Orchestrator agent persona.
...
The complete agent definition is available in [.claude/agents/task-orchestrator.md](.claude/agents/task-orchestrator.md).

## Usage

When the user types `@task-orchestrator`, activate this Task Orchestrator persona and follow all instructions defined in the YAML configuration above.
